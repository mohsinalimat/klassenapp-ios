...
class SettingsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let item10 = ExpandingMenuItem(size: menuButtonSize, title: "Terminal", image: UIImage(named: "terminal")!, highlightedImage: UIImage(named: "terminal")!, backgroundImage: UIImage(named: "terminal"), backgroundHighlightedImage: UIImage(named: "terminal")) { () -> Void in
            if let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "terminalID") as? TerminalViewController
            {
                self.present(vc, animated: true, completion: nil)
            }
            // Do some action
        ...
        if Auth.auth().currentUser != nil {
            menuButton.addMenuItems([item0, item1, item2, item3, item5, item6, item7, item8, item9])
        }
        else if UserDefaults.standard.string(forKey: "Terminal") == "enabled" {
            menuButton.addMenuItems([item0, item1, item2, item3, item5, item6, item7, item9, item10])
        }
        else {
